...
# ═══════════════════════════════════════════════════════════
# GET - TOUTES LES ALERTES (avec filtre)
# ═══════════════════════════════════════════════════════════
...
def get_alertes(
    ...
    """
    Filtre :
    - Alertes AUTO (FIN_CONVENTION, REUNION_COMITE) → tous les utilisateurs
    - Alertes MANUELLES → uniquement le créateur
    """
    query = db.query(Alerte)
    
    # ✅ FILTRE : exclure les manuelles des AUTRES utilisateurs
    query = query.filter(
        or_(
            Alerte.type_alerte != "MANUELLE",              # Auto → visible
            Alerte.created_by == current_user.id           # Manuelle créée par moi
        )
    )
    
    return query.order_by(Alerte.date_declenchement.desc()).all()
...
def create_alerte(
    ...
    alerte = Alerte(
        ...
        created_by=current_user.id                   # ✅ Créateur
    )
...
# ═══════════════════════════════════════════════════════════
# GET - Alertes par convention (avec filtre)
# ═══════════════════════════════════════════════════════════
...
def get_alertes_by_convention(
    ...
    query = db.query(Alerte).filter(Alerte.convention_id == convention_id)
    
    # ✅ Même filtre
    query = query.filter(
        or_(
            Alerte.type_alerte != "MANUELLE",
            Alerte.created_by == current_user.id
        )
    )
...
def update_alerte(
    ...
    if alerte.created_by != current_user.id and not current_user.is_admin:
        raise HTTPException(status_code=403, detail="Vous ne pouvez modifier que vos propres alertes")
...
async def check_expiration_alerts(
    ...
    """Vérifier et envoyer les alertes d'expiration"""
                ...
                if not existing:
                    ...
                    if comites:
                        ...
                        alerte = Alerte(
                            ...
                            envoyee=success,
                            traitee=success
                            # ⚠️ PAS de created_by pour les auto
                        )
                        db.add(alerte)
                        alerts_sent.append({
...
async def check_reunion_alerts(
    ...
    """Vérifier et envoyer les alertes de réunion"""
    email_service = EmailService()
    today = datetime.now().date()
...
# ═══════════════════════════════════════════════════════════
# POST - Envoyer alerte manuelle personnalisée (email direct)
# ═══════════════════════════════════════════════════════════
...
async def send_manual_alert(
    ...
    """Envoyer une alerte manuelle avec email direct"""
    email_service = EmailService()
